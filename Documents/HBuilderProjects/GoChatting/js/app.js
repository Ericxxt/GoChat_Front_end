window.app = {
	 ...
	 * netty服务后端发布地址
	 */
	nettyServerUrl : "ws://10.27.90.38:8088/ws",
	
	/**
	 * 服务后端发布地址
	 */
	serverUrl : "http://10.27.90.38:8080/",
	
	
	/**
	 * 图片服务器地址
	 */
	imgServerUrl : "http://10.27.90.38:8888/gochat/",
	 ...
	 * @param {Object} friendId
	 */
	// 这个标记已读的方法需要遍历所有聊天记录中朋友说的话，感觉效率有点低呀
	readUserChatSnapshot: function(userId, friendId){
		...
		if( this.isNotNull(chatSnapshotListStr) ){
			...
			for (var i = 0; i < chatSnapshotList.length; i++) {
				var snapshot = chatSnapshotList[i];
				// 确保是对方的信息才需要判断已读未读，所以需要snapshot.friendId == friendId
				if(snapshot.friendId == friendId){
